  ...
  render() {
    ...
    this.props.searchTerm.length>0 &&   <ScrollView>
          {this.props.searchItems.map((item,index) => {
            return (
              (this.props.searchItems.length>0 &&item._id!=userId._55)?<TouchableOpacity onPress={()=>this.viewSeller(item.brandName,item._id)} key={item._id} style={styles.emailItem}>
