...
class School extends Component {
    ...
    getItemSchool = async () => {
        ...
        try {
            // this.setState({loading:true})
            const response = await axios.get(`https://slick-project.herokuapp.com/api/search/availableInSchool?searchTerm=${this.props.searchTerm}&searchSchool=${this.state.schoolSearchText}`,
                {
    ...
    onInputChange = async (text) => {
        await this.setState({ schoolSearchText: text })
        this.getItemSchool()
    }
    ...
    render() {
        return (
            <View>
                ...
                {(this.state.schoolSearchText.length === 0 && this.props.searchTerm.length > 0) && <ScrollView>
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        ...
                                        <View>
                                            ...
                                            <Text style={styles.emailSubject}>{item.school}</Text>
                ...
                {(this.state.schoolSearchText.length > 0) &&
                     <ScrollView>
                ...
                {this.state.loading?<View><Text style={{color:'grey'}}>Searching for {this.props.searchTerm} in {this.state.schoolSearchText}..</Text></View>:null}
                {(this.state.finishedSchool&&this.state.searchAvailableInItems.length==0)&&<View style={{padding:wp('2')}}><Text style={{fontSize:wp('4.5'),color:'grey'}}>No item with that particular school was found</Text></View>}
